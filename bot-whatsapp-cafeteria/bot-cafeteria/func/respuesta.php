...
class Respuesta
{
    ...
    {
        $consulta = $sesion['datos']['consulta'];
        if ($consulta !== false) {
            $respuesta = call_user_func($consulta, $sesion, $data);
            $respuestaFormateada = $sesion['datos']['formartear_respuesta'];
            if ($respuestaFormateada !== false) {
            ...
            $mensaje = $sesion['datos']['respuesta_ok'];
    ...
    static public function formularioCompleto($arrayJson, $ids)
    {
        ...
        if ($camposLlenos === $camposParaLlenar) {
